...
const zEnv = z.object({
  ...
  FASTIFY_FRONTEND_URL: zStr.url().default('http://localhost:5173'),
  FASTIFY_SECRET: zStr,
  FASTIFY_PROXIES: z.union([z.number().nonnegative(), zStr.regex(/^\d+$/).transform(Number)]).default(0),
...
export const { NODE_ENV, FASTIFY_URL, FASTIFY_PORT, FASTIFY_FRONTEND_URL, FASTIFY_SECRET, FASTIFY_PROXIES } = parsedEnv;
